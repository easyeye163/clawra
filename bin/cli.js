...
// Check Stable Diffusion setup
async function checkStableDiffusion() {
  logStep("2/7", "Checking Stable Diffusion setup...");

  const SD_URL = "http://127.0.0.1:7860";

  log(`\nClawra Selfie uses local Stable Diffusion for image generation.`);
  log(`${c("cyan", "→")} Make sure Stable Diffusion WebUI is running at: ${c("bright", SD_URL)}\n`);

  // Test connection to Stable Diffusion API
  try {
    const response = await fetch(`${SD_URL}/sdapi/v1/sd-models`);
    if (response.ok) {
      logSuccess("Stable Diffusion WebUI is accessible!");
    } else {
      logWarn("Could not connect to Stable Diffusion WebUI");
      logInfo(`Please start Stable Diffusion WebUI and try again`);
    }
  } catch (error) {
    logWarn("Could not connect to Stable Diffusion WebUI");
...
async function main() {
  ...
  try {
    ...
    // Step 2: Check Stable Diffusion setup
    await checkStableDiffusion();
